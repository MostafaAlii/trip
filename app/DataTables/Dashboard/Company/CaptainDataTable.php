<?php
namespace App\DataTables\Dashboard\Company;
    ...
    public function dataTable($query): EloquentDataTable {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Captain $captain) {
                return view('dashboard.company.captains.btn.actions', compact('captain'));
            })
            ...
            ->editColumn('name', function (Captain $captain) {
                return '<a href="'.route('companyCaptains.show', $captain->profile->uuid).'">'.$captain->name.'</a>';
            })
