    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('general.edit') .' '. $employee->name }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{route('agentEmployees.update', $employee->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <!-- Start Avatar -->
                    {{--<div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" required name="name" id="name" value="{{$employee->name}}">
                        @error('name')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="country_id" class="p-1 form-control">
                            <option selected disabled>Select <span class="text-primary">{{$employee->name}}</span> Country...</option>
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="status" class="p-1 form-control">
                            <option selected disabled>Select <span class="text-primary">{{$employee->name}}</span>
                                Status...</option>
