<div class="modal fade" id="edit{{ $agent->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('general.edit') .' '. $agent->name }}</h5>
            ...
            <div class="modal-body">
                <form action="{{route('agents.update', $agent->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" required name="name" id="name" value="{{$agent->name}}">
                        @error('name')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="country_id" class="p-1 form-control">
                            <option selected disabled>Select <span class="text-primary">{{$agent->name}}</span> Country...</option>
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="status" class="p-1 form-control">
                            <option selected disabled>Select <span class="text-primary">{{$agent->name}}</span>
                                Status...</option>
                            <option value="active" {{ (old('status', $agent->status) == 'active') ? 'selected' : '' }}>
                                {{ trans('general.active') }}
                            </option>
                            <option value="inactive" {{ (old('status', $agent->status) == 'inactive') ? 'selected' : ''
                                }}>
